...
MODEL = "/home/muhdirfanrosdin/resAI/MCTA4363-Resilient-AI/models/gemma4-E4B-hybrid2.gguf"
...
def run_inference(image, question, idx=0):
    ...
    # Extract answer after thinking block ends - look for <channel|> marker
    if "<channel|>" in output:
        # Get everything after the closing channel tag
        output = output.split("<channel|>")[-1].strip()
    elif "<|channel>thought" in output:
        # Thinking didn't finish - get last non-bullet line
        after = output.split("<|channel>thought")[-1]
    ...
    # Clean up
    for marker in ["<|turn>model", "<turn|>", "<|turn>", "<|channel>"]:
...
print("\n--- EXPERIMENT 008: Hybrid2 Q8_embd+Q4_K_M_language (llama.cpp) ---")
...
report = {
    "model": "gemma-4-E4B-it Hybrid2 Q8_embd+Q4_K_M",
...
with open("008_results.json", "w") as f:
...
print(f"{'='*45}")
print("Saved to 008_results.json")